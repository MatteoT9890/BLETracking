...
public class RoomDetection {
		...
		private HashMap<String,String> MacToPosition;
		...
		public RoomDetection(int stopConversion){
			...
			ConversionRSSIMap = new HashMap<String,ConversionRSSI>();
			PositionMapping = new HashMap<String, ArrayList<RssiToMeters>>();
			MacToPosition = new HashMap<String,String>();
		...
		public void detect(Packet pack) {
			...
			if (!conversion.isCalibrato()){
				...
				if (rssiCalculated!=100){
					...
					this.MacToPosition.put(pack.getBeacon().getMAC(),position);
			...
			if (!this.MacToPosition.get(pack.getBeacon().getMAC()).equals(position2compare))	{
				System.out.println("MOVIMENTO RILEVATO! Calcolo nuova posizione del" + pack.getBeacon().getFeature());
				this.MacToPosition.put(pack.getBeacon().getMAC(), position2compare);
				printPosition(pack,position2compare);
			}
			}
			}
